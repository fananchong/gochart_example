...
var (
	playernum []int = []int{1, 5, 3, 6, 8, 0, 10, 11, 6, 10}
)
...
func (this *TimeChartExample) Update() {
	...
	json.Set("pointInterval", 1)
	json.Set("pointIntervalUnit", "second")
	json.Set("pointStart", uint(time.Now().Unix()-int64(len(playernum))))
	json.Set("pointEnd", uint(time.Now().Unix()))
